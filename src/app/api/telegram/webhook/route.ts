...
/**
 * POST /api/telegram/webhook — Fayek, the owner's Telegram assistant.
 *
...
const UNBOUND_START_NUDGE =
  "Hi — I'm Fayek, the owner's assistant. If you're the owner, start me with your access phrase like this:\n" +
  "/start <your access phrase>";

const BOUND_GREETING =
  "Bound and ready! 🤝 I'm Fayek, your ops assistant.\n\n" +
  "Ask me things like:\n" +
...
async function alertOwnerOfIntrusion(
  ...
  try {
    ...
    await sendMessage(
      owner,
      `⚠️ Someone tried to access Fayek — ${describeSender(strangerChatId, from)}, attempted: ${attempted}`
    );
  } catch (error) {
...
async function handlePhoto(
  ...
  // the in-store-sale flow (upload the photo, let Fayek record it behind the
