...
export default function OrderBookSection({ initial }: { initial: PurchaseOrder[] }) {
  ...
  return (
    <section>
      <h2 className="font-serif text-2xl text-[#0E2A47]">Order Book</h2>
      <p className="mt-1 text-sm text-[#5B7186]">
        All purchase orders across their lifecycle. In-flight value (open + invoiced):{" "}
        <span className="font-medium text-[#0E2A47]">{egp(openValue)}</span>
      </p>
    ...
    <div className="rounded-2xl border border-[#0E2A47]/10 bg-white px-4 py-3">
      ...
      {open && detail && (
        <div className="mt-3 border-t border-[#0E2A47]/10 pt-3">
          ...
          {detail.notes && <p className="mt-2 text-xs text-[#5B7186]">{detail.notes}</p>}
          {po.status === "open" && <p className="mt-2 text-xs text-[#8A5A12]">Awaiting invoicing &amp; fulfilment in Finance.</p>}
        </div>
      )}
    </div>
  );
