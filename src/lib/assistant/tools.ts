...
export const TOOLS: OllamaTool[] = [
  ...
  tool(
    ...
    {
      orderNumber: { type: "string", description: "e.g. FK-AB12CD" },
      status: {
        type: "string",
  ...
  tool(
    ...
    "Record a PHYSICAL in-store sale at the Egypt shop: it removes the item from inventory (catalog items only) AND counts as paid revenue (a delivered order). Use for walk-in / in-person sales (cash, card, InstaPay). Do NOT use log_income for items sold in person (that wouldn't reduce stock). TWO modes: (1) a WEBSITE product — give `product` (slug from catalog_list or exact name); or (2) a STORE-ONLY item NOT on the website — give `name` and `priceEgp` instead of `product` (this does not touch website stock). If the owner attached a photo of a store-only item, it is recorded automatically. MUTATING — requires the owner's button confirmation.",
    {
      product: {
        type: "string",
        description: "WEBSITE product: slug (from catalog_list) or exact product name",
      },
      name: {
        type: "string",
  ...
  tool(
    "order_lookup",
    "Full detail of ONE shop order by its FK-number: items, totals, address, contact, and complete status history with reasons.",
    {
      orderNumber: { type: "string", description: "e.g. FK-AB12CD" },
    },
    ["orderNumber"]
...
async function execOrderSetStatus(
  ...
  if (!isValidOrderNumber(orderNumber)) {
    return `Invalid order number "${orderNumber}" (expected FK-XXXXXX).`;
  }
...
async function execInStoreSale(args: Record<string, unknown>): Promise<string> {
  ...
  if (query) {
    ...
    if (!product) {
      ...
      if (exact.length === 1) product = exact[0];
        ...
        if (partial.length === 1) product = partial[0];
          ...
          return `Several products match "${query}": ${partial
            .map((p) => p.en.name)
            .join(", ")}. Which one?`;
      } else {
        return `Several products match "${query}". Use the exact name or the slug.`;
      }
    }
    if (!product)
      return `I couldn't find a product matching "${query}". Use catalog_list, or give a name + price to sell it as a store-only item.`;
    ...
    return "Tell me which product was sold (name or slug), or give a name + price for a store-only item.";
  }
  ...
  const record: StoredOrder = {
    ...
    note: `In-store sale — Egypt shop · Paid: ${paymentLabel}`,
    lang: "en",
    channel: "in_store",
    payment,
